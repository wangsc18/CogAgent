...
1.  **识别所有任务**: 结合屏幕截图和**所有打开的窗口标题**，全面分析并列出用户当前可能正在处理的**多项任务**。（例如：正在VS Code中编写Python代码、同时在Chrome中查阅文档、并且后台开着邮件客户端）。

2.  **决策建议逻辑**: 请遵循以下决策树来生成你的建议：
    a. **首先，寻找直接解决方案**: 遍历你识别出的所有任务，检查**可用工具集**中是否有任何一个工具能够**直接地、完整地**帮助完成其中**一项任务**。
       - **例如**: 如果用户正在查看一张流程图的图片，并且工具集里有 `generate_code_from_image` 工具，这就是一个直接的解决方案。
       - **如果找到**，你的建议**必须**聚焦于使用这个工具来解决该特定任务。

    b. **其次，提供通用帮助 (回退策略)**: 如果**没有**任何工具能直接解决一个完整的任务，请检查用户是否正在进行可以被**文件分析**所帮助的活动（例如：编写代码、阅读PDF文档、查看长文本）。
...
{{
  ...
  "recommended_tool": "如果你的建议是方案`2.a`（直接解决方案），请在此处填写对应的工具名称。如果建议是方案`2.b`（邀请上传文件），请将此字段的值设为 `null`。",
  "reasoning": "解释你为什么会提出这个建议的简短理由，点明是基于直接工具匹配还是回退策略。"
}}
"""
        multimodal_content = [
